...
export default function JobDetail({ params }) {
  ...
  if (loading) return <p style={{textAlign:'center', padding:'60px', color:'#6b7280'}}>Loading...</p>
  if (!job) return <p style={{textAlign:'center', padding:'60px', color:'#6b7280'}}>Job not found.</p>
  ...
  return (
    <div style={{maxWidth:'800px', margin:'0 auto', padding:'24px 16px'}}>
      ...
      <div style={{background:'white', borderRadius:'16px', padding:'28px', marginTop:'16px', border:'1px solid #e5e7eb', boxShadow:'0 2px 8px rgba(0,0,0,0.07)'}}>
        {job.is_featured && (
          <span style={{background:'#fef3c7', color:'#92400e', fontSize:'12px', padding:'3px 10px', borderRadius:'20px', fontWeight:'500'}}>⭐ Featured Job</span>
        )}
